...
import { queryAgent, getUsage, resetUsage } from "./llm/client.js";
import { resolveConsensus } from "./consensus.js";

const agents: Agent[] = [melchior, balthasar, casper];

/** Round 1: Each agent gives their independent opinion */
async function round1(question: string, model?: string): Promise<RoundResult> {
  const votes = await Promise.all(
    agents.map(async (agent) => ({
      agent,
      response: await queryAgent(agent, question, model),
    }))
  );
  return { round: 1, label: "Independent Analysis", votes };
}

/** Round 2: Each agent reviews others' opinions and may revise */
async function round2(
  ...
  const votes = await Promise.all(
    agents.map(async (agent) => {
      // Collect other agents' opinions
      const othersOpinions = r1.votes
        .filter((v) => v.agent.name !== agent.name)
        .map(
          (v) =>
            `${v.agent.icon} ${v.agent.name} (${v.agent.codename}) voted ${v.response.vote.toUpperCase()}:\n"${v.response.reasoning}"`
        )
        .join("\n\n");
...
Respond ONLY with the JSON object, no other text.`;
      ...
      return {
        agent,
        response: await queryAgent(agent, deliberationPrompt, model),
      };
    })
  );
...
/** Round 3: Final vote after seeing revised positions */
  ...
  const votes = await Promise.all(
    agents.map(async (agent) => {
      ...
      const r2Opinions = r2.votes
        .map(
          (v) =>
            `${v.agent.icon} ${v.agent.name}: ${v.response.vote.toUpperCase()} - "${v.response.summary}"`
        )
        .join("\n");
...
Respond ONLY with the JSON object, no other text.`;
      ...
      return {
        agent,
        response: await queryAgent(agent, finalPrompt, model),
      };
    })
  );
...
export async function deliberate(question: string, opts: DeliberateOptions = {}) {
  const { model, rounds = 3 } = opts;
  resetUsage();

  const r1 = await round1(question, model);

  if (rounds === 1) {
    return resolveConsensus(r1.votes, [r1], getUsage());
  }

  const r2 = await round2(question, r1, model);
  const r3 = await round3(question, r1, r2, model);
